...
import { X, Check, Info, Send, EyeOff, SlidersHorizontal, ListChecks, Star, ChevronRight, ChevronLeft, LayoutGrid, Sparkles, ArrowDown, Clock, Sun, Moon } from 'lucide-react';
...
const SubscriptionModal: React.FC<SubscriptionModalProps> = ({ isOpen, onClose }) => {
  ...
  // Reset scroll to top and check for scrollability on step change
  useEffect(() => {
    if (scrollContainerRef.current) {
      scrollContainerRef.current.scrollTo({ top: 0, behavior: 'instant' });
      
      const checkScrollable = () => {
        if (scrollContainerRef.current && (step === 1 || step === 3)) {
  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-0 md:p-6 lg:p-12">
        ...
        >
          {/* Header */}
          <div className="flex flex-col border-b border-gray-100 dark:border-neutral-800 shrink-0 bg-white/80 dark:bg-neutral-950/80 backdrop-blur-md z-30">
            <div className="flex justify-between items-center p-6 pb-4">
              <div className="flex items-center gap-3">
                 <img src="https://i.postimg.cc/SxPbst4r/Fusion-Bowl-PNG-(Bg-removed).png" alt="Logo" className="w-8 h-8 object-contain" />
                 <span className="font-serif font-bold text-xl dark:text-brand-cream">Fusion<span className="text-brand-orange">Bowl</span></span>
              </div>
            ...
            <div className="px-6 pb-4 flex items-center justify-center gap-2">
                {[1, 2, 3, 4].map(i => (
                  <div key={i} className="flex items-center gap-2">
                    <div className={`h-2 rounded-full transition-all duration-500 ${step >= i ? 'w-10 bg-brand-orange' : 'w-4 bg-gray-100 dark:bg-neutral-800'}`} />
                    {i < 4 && <div className="w-1.5 h-0.5 bg-gray-200 dark:bg-neutral-800" />}
                  </div>
                ))}
            </div>
          </div>
          ...
          <div className="flex-grow overflow-hidden relative">
            <div 
              ...
              className="absolute inset-0 overflow-y-auto custom-scrollbar p-6 md:p-10"
            >
                  ...
                  {step === 1 && (
                    <div className="space-y-8 pb-12">
                      <div className="text-center md:text-left">
                        ...
                        <p className="text-gray-500 dark:text-gray-400">Select a membership plan that fits your nutrition goals.</p>
                      </div>
                      ...
                      <div className="grid grid-cols-1 gap-4">
                        {(Object.keys(PLAN_FEATURES) as PlanType[]).map((type) => {
                          ...
                          return (
                            <motion.div 
                              key={type} 
                              onClick={() => { setPlanType(type); setExclusions({}); }}
                            ...
                            >
                              <div className="flex justify-between items-start mb-4">
                                <div>
                                  <h4 className={`text-xl font-bold ${isActive ? 'text-brand-orange' : 'text-gray-800 dark:text-gray-200'}`}>{type} Membership</h4>
                                  <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-1">Curated Selection</p>
                                </div>
                  ...
                  {step === 2 && (
                    <div className="space-y-10 pb-12">
                            ...
                            {(['15 Days', '1 Month'] as PlanDuration[]).map((d) => (
                              <button key={d} onClick={() => setDuration(d)}
                                className={`group p-6 rounded-3xl font-bold transition-all border-2 text-center relative overflow-hidden ${
                                  duration === d ? 'bg-brand-orange border-brand-orange text-white shadow-xl shadow-orange-500/20 scale-[1.02]' : 'bg-white dark:bg-neutral-900 border-gray-100 dark:border-neutral-800 text-gray-500 hover:border-brand-orange/30'
                                }`}
                              >
                            ...
                            {(['Mini', 'Compact', 'Grand'] as PlanSize[]).map((s) => (
                              <button key={s} onClick={() => setSize(s)}
                                className={`p-6 rounded-[2rem] font-bold transition-all border-2 text-left flex justify-between items-center group ${
                                  size === s ? 'bg-brand-green border-brand-green text-white shadow-xl shadow-green-500/20' : 'bg-white dark:bg-neutral-900 border-gray-100 dark:border-neutral-800 text-gray-500 hover:border-brand-green/30'
                                }`}
                              >
                                <div className="flex flex-col">
                                  <span className="text-xl">{s}</span>
                                  <span className={`text-[11px] font-medium opacity-80 ${size === s ? 'text-white' : 'text-gray-400'}`}>{SIZE_DETAILS[s]}</span>
                                </div>
                      ...
                      <div className="p-6 bg-brand-cream dark:bg-neutral-900 rounded-[2rem] border border-gray-100 dark:border-neutral-800 flex gap-4">
                        <div className="p-2 bg-brand-orange/10 rounded-xl h-fit"><Info className="w-5 h-5 text-brand-orange" /></div>
                        <div className="text-xs text-gray-600 dark:text-gray-400 leading-relaxed">
                          <span className="font-bold text-brand-dark dark:text-brand-cream block mb-1 underline">Serving Schedule</span>
                          To maintain the highest hygiene and quality, we observe Sundays as restful holidays. 
                          <div className="mt-2 font-medium bg-white/50 dark:bg-neutral-800/50 p-3 rounded-xl border border-dashed border-brand-orange/20">
                            • A 15-day membership provides <span className="text-brand-orange font-bold">13 fresh bowls</span>.<br/>
                            • A monthly membership provides <span className="text-brand-orange font-bold">26 fresh bowls</span>.
                          </div>
                        </div>
                      </div>
                    </div>
                  ...
                  {step === 3 && (
                    <div className="space-y-8 pb-12">
                      <div className="text-center md:text-left">
                        ...
                        <h3 className="text-3xl md:text-4xl font-serif font-bold text-brand-dark dark:text-brand-cream mb-4">Delivery Window</h3>
                        <p className="text-gray-500 dark:text-gray-400">Select your preferred time slot for daily delivery.</p>
                      </div>
                      ...
                      <div className="space-y-10">
                         <div>
                            <div className="flex items-center gap-2 mb-4 px-1">
                               ...
                               <label className="text-[10px] font-black uppercase text-brand-orange tracking-[0.2em]">Morning Slots (07:00 - 11:00)</label>
                            </div>
                         ...
                         <div>
                            <div className="flex items-center gap-2 mb-4 px-1">
                               ...
                               <label className="text-[10px] font-black uppercase text-blue-500 tracking-[0.2em]">Evening Slots (04:00 - 07:00)</label>
                            </div>
                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                               {EVENING_SLOTS.map(slot => (
                                 <button
                                   ...
                                   className={`p-4 rounded-2xl text-sm font-bold border-2 transition-all ${
                                      ...
                                      : 'bg-white dark:bg-neutral-900 border-gray-100 dark:border-neutral-800 text-gray-500 hover:border-blue-500/30'
                                   }`}
                                 >
                                   {slot}
                                 </button>
                  ...
                  {step === 4 && (
                    <div className="space-y-8 flex flex-col h-full pb-12">
                       <div className="text-center md:text-left">
                          ...
                          <h3 className="text-3xl md:text-4xl font-serif font-bold text-brand-dark dark:text-brand-cream mb-4">Personalization</h3>
                          <p className="text-gray-500 dark:text-gray-400">Tap items to <strong>exclude</strong> them from your daily rotation.</p>
                       </div>
                                              ...
                                              return (
                                                  <motion.button key={item.name} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                                                    ...
                                                    className={`flex items-center gap-2.5 px-4 py-2.5 rounded-2xl text-xs font-bold border-2 transition-all duration-300 ${
                                                        isExcluded ? 'bg-gray-50 dark:bg-neutral-900 text-gray-400 border-dashed border-gray-200 dark:border-neutral-800' : 'bg-white dark:bg-neutral-900 text-gray-700 dark:text-gray-200 border-gray-100 dark:border-neutral-800 hover:border-brand-orange shadow-sm'
                                                    }`}
                                                  >
            ...
            <AnimatePresence>
              {showScrollHint && (
                ...
                >
                  <div className="bg-brand-orange text-white px-5 py-2.5 rounded-full shadow-2xl flex items-center gap-3 border border-orange-400">
                    <div className="bg-white/20 p-1 rounded-full"><ArrowDown size={14} className="animate-bounce" /></div>
                    <span className="text-[11px] font-black uppercase tracking-wide whitespace-nowrap">
                      {step === 1 ? 'Explore All Membership Plans' : 'View All Delivery Windows'}
                    </span>
                  </div>
                </motion.div>
              )}
          ...
          {/* Navigation Footer */}
          <div className="p-6 md:p-8 border-t border-gray-100 dark:border-neutral-800 bg-white dark:bg-neutral-950 shadow-[0_-20px_50px_rgba(0,0,0,0.05)] shrink-0 z-40">
            <div className="max-w-2xl mx-auto">
              <div className="flex justify-between items-end mb-8">
                 ...
                 <div className="text-right hidden sm:block">
                    ...
                    <span className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">{selectedSlot}</span>
                 </div>
              </div>
              <div className="flex gap-3">
                 {step > 1 && (
                    <button onClick={handleBack} className="flex-1 flex items-center justify-center gap-2 py-4 rounded-full border-2 border-gray-100 dark:border-neutral-800 font-bold text-gray-500 hover:bg-gray-50 dark:hover:bg-neutral-900 transition-all">
                      <ChevronLeft size={18} /> Back
                    </button>
                 )}
                 {step < 4 ? (
                   <Button onClick={handleNext} className="flex-[2] gap-2 py-4 shadow-xl">Continue <ChevronRight size={18} /></Button>
                 ) : (
                   <Button onClick={handleSubscribe} className="flex-[2] gap-3 py-4 text-lg shadow-2xl"><Send className="w-5 h-5" /> Confirm Membership</Button>
                 )}
              </div>
            </div>
          </div>
