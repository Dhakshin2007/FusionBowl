...
const BowlBuilder: React.FC = () => {
  ...
  const renderVisuals = () => {
      if (currentView === 'shake') {
          ...
          // Determine dynamic color based on the first item (or default)
          const baseColor = currentIngredients.length > 0 
            ? currentIngredients[0].item.color.replace('bg-', 'from-') 
            : 'from-orange-200';

          return (
              <div className="relative w-full h-full overflow-hidden rounded-b-[3.5rem] rounded-t-sm bg-blue-50/50 dark:bg-blue-900/10">
                  <motion.div 
                    className={`absolute bottom-0 left-0 right-0 bg-gradient-to-t ${baseColor} to-white/50 dark:to-white/10 opacity-90`}
